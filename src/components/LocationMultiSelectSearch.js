...
const LocationMultiSelectPopup = ({ name, modalVisible, setModalVisible, locationData, onCallBack, placeholder = "",selectedValue }) => {
    ...
    //     selectedData[index].selected = selectedData[index].selected ? !selectedData[index].selected : true;
    //     return setData([...selectedData]);
    // }

    const onUpdateValue = (index, value) => {
        const findIndex = selectedData.findIndex((item) => item == value)
        console.log('findIndex', findIndex)
        if (findIndex === -1) {
            setSelectedData([...selectedData, value])
        } else {
            selectedData.splice(findIndex, 1)
            console.log('filteredList', selectedData)
    ...
    const renderItem = ({ item, index }) => {
        const id = item.value
        const isSelecetd = selectedData && selectedData.find((item) => item == id)
        return (
            <View style={[styles.dropdown, { backgroundColor: isSelecetd ? COLORS.primary : COLORS.secondary }]}>
                <TouchableOpacity onPress={() => {
                    Keyboard.dismiss()
                    //  console.log('selected item', item)
                    //  setModalVisible(!modalVisible)
                    onUpdateValue(index, item.value)
                }}>
...
export default LocationMultiSelectPopup;
